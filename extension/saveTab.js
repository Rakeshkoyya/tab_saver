    ...
    */
    ...
    const importTabs = (fileInput) => {
        ...
        function createButtons(data) {
            ...
            Object.keys(data).forEach(function (filename) {
                ...
                buttonContainer.classList.add('importContainer');
    
                var filenameButton = document.createElement("button");
                filenameButton.classList.add('import');
                filenameButton.classList.add('buttonX');
                ...
                deleteButton.classList.add('import');
                deleteButton.classList.add('buttonY');
    ...
    const importTabsWithFilename = (filename) => {
        ...
        chrome.storage.local.get('fileDataObj', async function(result) {
            ...
            // console.log(fileDataObj[0]);


                
            
        // const fileName = "Save_Tabs_12-4-2023_1-25-28_AM.json"
        // Retrieve the data from chrome.storage.local with the filename 'newFile'
        // chrome.storage.local.get(fileName, function(result) {
            // const bblob = fileDataObj[0];
            // const jsonString = await bblob.text();
            ...
            if (fileContent) {
                ...
                browserAPI.runtime.sendMessage(
                    ...
                    (response) => {
                        try {
                            console.log(response);
                            // Close popup in Chrome. No effect on Firefox sidebar.
                            // If the popup remains, one of the observed issues is on importing tabs (all of them grouped) and then importing a new file, it doesn't get imported.
                            // Need to check if this can be better handled by handling the file reader in a more efficient manner.
                            window.close();
                        } catch (error) {
                            console.error(error);
                        }
    ...
    // Assuming that you call importTabs at some point in your code
    // importTabs();
    ...
    const downloadFile = (tabs) => {
         ...
         */
        const initiateDownload = async (fileContent) => {
    ...
    const exportTabs2 = () => {
        ...
        browserAPI.tabs
            ...
            .then(exportToLocal, logErrorOrSuccess.bind(null, `error`));
    };
    ...
    const init = () => {
        ...
        document.getElementById(`localExport`).addEventListener(`click`, exportTabs2);
